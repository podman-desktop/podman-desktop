"use strict";
(globalThis["webpackChunkdocs"] = globalThis["webpackChunkdocs"] || []).push([[34363],{
...
function map(collection, iteratee) {
  var func = (0,isArray/* default */.A)(collection) ? _arrayMap/* default */.A : _baseMap;
  return func(collection, (0,_baseIteratee/* default */.A)(iteratee, 3));
}
...
function greedyFAS(g, weightFn) {
  ...
  return lodash_es_flatten(
    lodash_es_map(results, function (e) {
...
function assignValue(object, key, value) {
  ...
  if (!(_assignValue_hasOwnProperty.call(object, key) && (0,eq/* default */.A)(objValue, value)) ||
      (value === undefined && !(key in object))) {
    _baseAssignValue(object, key, value);
  }
}
...
var _arrayLikeKeys = __webpack_require__(78373);
...
var _castPath = __webpack_require__(57272);
...
function buildLayerMatrix(g) {
  var layering = lodash_es_map(lodash_es_range(util_maxRank(g) + 1), function () {
    return [];
  });
  forEach/* default */.A(g.nodes(), function (v) {
    var node = g.node(v);
...
function normalizeRanks(g) {
  var min = lodash_es_min(
    lodash_es_map(g.nodes(), function (v) {
...
function longestPath(g) {
  ...
  function dfs(v) {
    ...
    var rank = lodash_es_min(
      lodash_es_map(g.outEdges(v), function (e) {
...
var graph = __webpack_require__(6756);
...
function baseAssign(object, source) {
  return object && _copyObject(source, (0,keys/* default */.A)(source), object);
}
...
function copySymbols(source, object) {
  return _copyObject(source, (0,_getSymbols/* default */.A)(source), object);
}
...
function baseClone(value, bitmask, customizer, key, object, stack) {
  ...
  var keysFunc = isFull
    ...
    : (isFlat ? lodash_es_keysIn : keys/* default */.A);
...
function twoLayerCrossCount(g, northLayer, southLayer) {
  ...
  var southPos = lodash_es_zipObject(
    southLayer,
    lodash_es_map(southLayer, function (v, i) {
      return i;
    }),
  );
  var southEntries = lodash_es_flatten(
    lodash_es_map(northLayer, function (v) {
  ...
  firstIndex -= 1;
  var tree = lodash_es_map(new Array(treeSize), function () {
    return 0;
  });
...
function initOrder(g) {
  ...
  var maxRank = lodash_es_max(
    lodash_es_map(simpleNodes, function (v) {
      return g.node(v).rank;
    }),
  );
  var layers = lodash_es_map(lodash_es_range(maxRank + 1), function () {
    return [];
  });
...
function barycenter(g, movable) {
  return lodash_es_map(movable, function (v) {
    var inV = g.inEdges(v);
    if (!inV.length) {
      return { v: v };
    } else {
...
function buildLayerGraphs(g, ranks, relationship) {
  return lodash_es_map(ranks, function (rank) {
...
function forIn(object, iteratee) {
  return object == null
    ? object
    : (0,_baseFor/* default */.A)(object, (0,_castFunction/* default */.A)(iteratee), lodash_es_keysIn);
}
...
function balance(xss, align) {
  return lodash_es_mapValues(xss.ul, function (ignore, v) {
    if (align) {
      ...
      var xs = lodash_es_sortBy(lodash_es_map(xss, v));
      return (xs[1] + xs[2]) / 2;
    }
  });
}
...
function positionX(g) {
  ...
  forEach/* default */.A(['u', 'd'], function (vert) {
    ...
    forEach/* default */.A(['l', 'r'], function (horiz) {
      if (horiz === 'r') {
        adjustedLayering = lodash_es_map(adjustedLayering, function (inner) {
...
function positionY(g) {
  ...
  forEach/* default */.A(layering, function (layer) {
    var maxHeight = lodash_es_max(
      lodash_es_map(layer, function (v) {
