...
export class DropdownComponent {
  ...
  /**
   * Check if the dropdown is currently open by looking for the options container
   * @returns True if the dropdown options are visible
   */
  async isOpen(): Promise<boolean> {
    // The options list appears as a div with absolute positioning when dropdown is open
    const optionsList = this.containerLocator.locator('div').filter({ hasText: /.*/ }).first();
    return await optionsList.isVisible();
  }
  ...
  async getAvailableOptions(): Promise<string[]> {
    return test.step('Get available dropdown options', async () => {
      ...
      // Filter out the trigger button text (first button)
      const triggerText = await this.triggerButton.innerText();
      const options = buttonTexts.filter(text => text !== triggerText);
  ...
  async verifyState(expectedValue: string, expectedDisplayText?: string): Promise<void> {
    return test.step(`Verify dropdown state: ${expectedValue}`, async () => {
      await playExpect(this.hiddenInput).toHaveValue(expectedValue, { timeout: 5_000 });
