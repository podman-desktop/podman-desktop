...
test.describe.serial('Podman installer integration in Podman Desktop', { tag: '@update-install' }, () => {
  ...
  test('Podman installer artifacts are present in local assets storage', async ({ runner }) => {
    ...
    const podmanInstallerFilePrefix = `podman-installer-${isWindows ? 'windows' : 'macos'}`;
    console.log(
    ...
    // windows file check: "podman-installer-windows-arch().exe", on mac: "podman-installer-macos-(universal|arch()-*.pkg"
