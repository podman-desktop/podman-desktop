...
test.describe.serial('Kubernetes newtworking E2E tests', { tag: '@k8s_e2e' }, () => {
  test('Check Ingress controller pods status', async ({ page }) => {
    ...
    await monitorPodStatusInClusterContainer(page, KIND_NODE, COMMAND_TO_EXECUTE, 'envoy', 'Running', '2/2');
    await monitorPodStatusInClusterContainer(page, KIND_NODE, COMMAND_TO_EXECUTE, 'contour', 'Running', '1/1');
    await monitorPodStatusInClusterContainer(
      page,
      KIND_NODE,
      COMMAND_TO_EXECUTE,
      'contour-certgen',
