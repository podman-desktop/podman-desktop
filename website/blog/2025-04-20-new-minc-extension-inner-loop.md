...
The CRI-O project implements the CRI interface and reuses image management from [https://github.com/containers/image](https://github.com/containers/image) project and handles storage using [https://github.com/containers/storage](https://github.com/containers/storage) project.
...
A key benefit is that the MINC container shares the image storage folder used by Podman. So when you build an image with Podman, it's instantly available in the cluster, no uploading or copying required.
...
Using MINC and the Minc extension significantly reduces turnaround time between building and testing container images.
