...
| `providers.disableUpdate`                    | array   | `[]`               | List of provider IDs to prevent from registering and displaying engine updates (e.g., `["podman", "lima"]`). Use `["*"]` to disable updates for all providers. This is a hidden setting intended for [managed configuration](/docs/configuration/managed-configuration)                                              |
