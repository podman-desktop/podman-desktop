...
New Podman Desktop installations (since 1.21) use the `libkrun` hypervisor by default. It is not compatible with Rosetta
and uses [qemu](https://www.qemu.org/) for `x86_64`. Rosetta support is enabled by default on all new installations,
but you need to recreate your Podman machine to use the `applehv` hypervisor.

#### Prerequisites

- Mac with Apple silicon
- Podman Desktop 1.11 or later
- Podman CLI 5.1.0 or later
