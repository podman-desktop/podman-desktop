...
- A runtime endpoint for the Container Runtime Interface (CRI)

List of common sockets and container runtimes, that can be installed and configured:

| Docker API socket | Kubernetes CRI socket | Notes     |
| ----------------- | --------------------- | --------- |
| nerdctl.sock      | containerd.sock       | _Default_ |
...
1. Push your image to your Lima cluster (if needed):
   1. Click the **overflow menu** icon corresponding to `my-custom-image` and select **Push image to Lima cluster**. A successful operation notification opens.
      ![pushing an image to Lima](img/pushing-an-image-to-lima.png)
   1. Click **OK**.
...
1. Test your image and container on your Lima cluster:
   ...
   1. Click **Deploy**. Wait for the pod to reach the state: **Phase: Running**.
      ![deploy button](img/deploy-button-lima.png)
   1. Click **Done**.
...
- [Container Runtimes](https://kubernetes.io/docs/setup/production-environment/container-runtimes/) setup documentation for Kubernetes
- [nerdctl daemon](https://github.com/afbjorklund/nerdctld) (`nerdctl.sock` for containerd/buildkit)
