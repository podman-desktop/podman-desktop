...
describe('applyInsertSlotReorder', () => {
  ...
  test('returns unchanged ids for an invalid source index or id', () => {
    expect(applyInsertSlotReorder(['A', 'B'], 'C', 0, 2)).toEqual(['A', 'B']);
    expect(applyInsertSlotReorder(['A', 'B'], 'A', 5, 0)).toEqual(['A', 'B']);
  });
});
