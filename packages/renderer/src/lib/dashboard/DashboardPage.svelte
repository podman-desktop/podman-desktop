<script lang="ts">
import { type LayoutEditItem, LayoutManager, NavPage } from '@podman-desktop/ui-svelte';
...
// Dashboard section configuration managed by backend
let dashboardSections: LayoutEditItem[] = [];
let isInitialized = false;

// Available dashboard sections
const availableSections = ['release-notes', 'extension-banners', 'learning-center', 'providers'];

onMount(async () => {
  // Set defaults and load dashboard configuration
  await window.setTableDefaults('dashboard', availableSections);
  dashboardSections = (await window.loadTableConfig('dashboard', availableSections)) || [];
  isInitialized = true;
});

// Save configuration whenever dashboardSections changes (after initialization)
$: if (isInitialized && dashboardSections.length > 0) {
  window.saveTableConfig('dashboard', dashboardSections).catch((error: unknown) => {
...
async function resetDashboardLayout(): Promise<void> {
  dashboardSections = (await window.resetTableConfig('dashboard', availableSections)) || [];
}
