...
let notificationStyleMap = {
  info: {
    color: '[var(--pd-state-info)]',
    icon: faCircleInfo,
  },
  warn: {
    color: '[var(--pd-state-warning)]',
    icon: faExclamationTriangle,
  },
  error: {
    color: '[var(--pd-state-error)]',
    icon: faCircleExclamation,
  },
};

let { [notification.type]: notificationStyle } = notificationStyleMap;
</script>

<div
  class="bg-[var(--pd-content-card-bg)] text-[var(--pd-content-card-text)] p-4 w-full border-{notificationStyle.color} border-t-[3px]"
  role="region"
  aria-label="id: {notification.id}">
  <div class="flex flex-row space-x-3">
    <div class="flex">
        <Icon size="1.5x" icon={notificationStyle.icon} class="text-{notificationStyle.color}" />
    </div>
