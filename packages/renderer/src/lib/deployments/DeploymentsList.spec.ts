...
import * as resourcesList from '../kube/resources-list';
import DeploymentsList from './DeploymentsList.svelte';

vi.mock('/@/stores/kubernetes-contexts-state');
vi.mock('../kube/resources-list');
...
describe.each<{
...
}>([
  {
    experimental: true,
    initMocks: (): void => {
      vi.mocked(resourcesList.listenResources).mockResolvedValue([]);
    },
    initObjectsList: (_objects: KubernetesObject[]): { update: (objects: KubernetesObject[]) => void } => {
      return {
        update: (_objects: KubernetesObject[]): void => {},
      };
    },
  },
  {
...
])('kubernetes experimental is %s', ({ experimental, initObjectsList, initMocks }) => {
  beforeEach(() => {
    vi.mocked(window.getConfigurationValue).mockResolvedValueOnce(experimental);
    initMocks();
  });
