...
<div class="flex flex-col w-full m-4 p-4 rounded-lg bg-[var(--pd-content-card-bg)]">
  ...
  <div class="h-full overflow-auto p-2 bg-[var(--pd-invert-content-card-bg)]">
    ...
    {#each info?.healthCheckers ?? [] as healthchecker (healthchecker)}
      <ul>
    ...
    <div class="flex flex-col space-y-4">
      {#each info?.permissionCheckers ?? [] as permissionChecker (permissionChecker)}
        <ul>
    ...
    <div class="flex flex-col space-y-4">
    {#each info?.informers ?? [] as informer (informer)}
      <ul>
