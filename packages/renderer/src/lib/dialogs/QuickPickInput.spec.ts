...
beforeAll(() => {
  ...
  // We use spyOn to ensure that the values are undefined / reset before tests.
  vi.spyOn(window, 'sendShowQuickPickOnSelect').mockImplementation(sendShowQuickPickOnSelectMock);
  vi.spyOn(window, 'sendShowQuickPickValues').mockImplementation(sendShowQuickPickValuesMock);
});
