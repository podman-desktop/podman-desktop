...
describe('Command Palette', () => {
  ...
  test('Check tab key switches search options', async () => {
    render(CommandPalette, { display: true });
    ...
    // Check initial state - "All" should be selected (has selected styling)
    const allButton = screen.getByRole('button', { name: /All/ });
    expect(allButton).toHaveClass('text-[var(--pd-button-tab-text-selected)]');

    // now, press the Tab key
    await userEvent.keyboard('{Tab}');

    // Check that "Commands" is now selected
    const commandsButton = screen.getByRole('button', { name: /Commands/ });
    expect(commandsButton).toHaveClass('text-[var(--pd-button-tab-text-selected)]');

    // Check that "All" is no longer selected
    expect(allButton).not.toHaveClass('text-[var(--pd-button-tab-text-selected)]');
  });
