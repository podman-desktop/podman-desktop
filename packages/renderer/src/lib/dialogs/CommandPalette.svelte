...
let inputValue: string | undefined = $state(searchOption);
let strippedInputValue = $derived(inputValue?.replace(searchOption, ''));
...
let filteredCommandInfoItems: CommandInfo[] = $derived.by(() =>
  commandInfoItems
    ...
    .filter(item => (strippedInputValue ? item.title?.toLowerCase().includes(strippedInputValue.toLowerCase()) : true)),
);
