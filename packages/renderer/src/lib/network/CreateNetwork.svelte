...
import { Button, Checkbox, Dropdown, Input, Tab } from '@podman-desktop/ui-svelte';
...
// Default subnet used for ipvlan/macvlan when user doesn't specify one
// eslint-disable-next-line sonarjs/no-hardcoded-ip
const DEFAULT_SUBNET = '10.89.0.0/24';
...
// Check if subnet is required (for ipvlan and macvlan) - we'll use default if not provided
...
async function createNetwork(): Promise<void> {
  ...
  try {
    ...
    // Determine the subnet to use - use default for ipvlan/macvlan if not specified
    const subnetToUse = networkInfo.subnet || (requiresSubnet ? DEFAULT_SUBNET : '');

    // Build IPAM config if subnet is provided or required
    ...
    {#snippet content()}
      <div class="space-y-2">
        <div class="flex flex-row px-2 border-b border-[var(--pd-content-divider)]">
          ...
          <Tab title="Advanced" selected={isTabSelected($router.path, 'advanced')} url={getTabUrl($router.path, 'advanced')} />
        </div>
        <div>
