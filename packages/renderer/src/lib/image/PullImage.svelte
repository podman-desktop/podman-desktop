...
async function resolveShortname(): Promise<void> {
  ...
  if (imageToPull && !imageToPull.includes('/')) {
    shortnameImages =
      (await window.resolveShortnameImage($state.snapshot(selectedProviderConnection), $state.snapshot(imageToPull))) ??
      [];
    // not a shortname
  } else {
    podmanFQN = '';
