...
function getUpdateAvailableStyle(status?: ImageUpdateStatus): LabelStatusStyle {
  if (!status) {
    return {
  ...
  {#snippet content()}
    <div class="flex flex-col gap-4">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-[var(--pd-table-body-text)]">
          ...
          <tbody>
            {#each imagesToUpdate as info (info.image.id + info.image.tag)}
              {@const imageDisplay = `${info.image.name}:${info.image.tag}`}
              {@const updateStyle = getUpdateAvailableStyle(info.status)}
              {@const resultStyle = getResultStyle(info)}
              <tr class="border-b border-[var(--pd-table-body-text-secondary)] bg-[var(--pd-content-bg)]">
                <td class="px-4 py-3 font-medium whitespace-nowrap" title={imageDisplay}>
                  {imageDisplay.length > 50 ? '...' + imageDisplay.slice(-47) : imageDisplay}
                </td>
                <td class="px-4 py-3">
                  <Label role="status" name={updateStyle.label}>
                    <div class="w-2 h-2 shrink-0 {updateStyle.dotColor} rounded-full mx-1"></div>
                  </Label>
                </td>
                <td class="px-4 py-3 max-w-xs truncate text-[var(--pd-table-body-text-secondary)]" title={info.status?.message ?? ''}>
                  {info.status?.message ?? '-'}
                </td>
                <td class="px-4 py-3 max-w-xs">
                  ...
                  {:else if resultStyle}
                    <Label role="status" name={resultStyle.label}>
                      <div class="w-2 h-2 shrink-0 {resultStyle.dotColor} rounded-full mx-1"></div>
                    </Label>
                    {#if info.error}
                      <div class="text-xs text-[var(--pd-status-terminated)] truncate mt-1" title={info.error}>
                        {info.error}
                      </div>
                    {/if}
                  {:else}
