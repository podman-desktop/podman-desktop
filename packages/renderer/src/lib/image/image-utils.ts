...
export class ImageUtils {
  ...
  getInUse(imageInfo: ImageInfo, repositoryTag?: string, containersInfo?: ContainerInfoUI[]): boolean {
    ...
    return containersInfo.some(container => {
      // imageId is optional on ContainerInfoUI, so an undefined one simply never matches:
      // falling back to another field here would report the wrong image as in-use
