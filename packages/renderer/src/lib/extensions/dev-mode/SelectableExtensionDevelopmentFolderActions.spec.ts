...
import SelectableExtensionDevelopmentFolderActions from './SelectableExtensionDevelopmentFolderActions.svelte';
...
test('Expect start action being displayed', async () => {
  render(SelectableExtensionDevelopmentFolderActions, { extensionFolder: extensionFolderWithExtensionStopped });
  const startButton = screen.getByRole('button', { name: 'Start the extension' });
  expect(startButton).toBeInTheDocument();
});

test('Expect stop action being displayed', async () => {
  render(SelectableExtensionDevelopmentFolderActions, { extensionFolder: extensionFolderWithExtensionStarted });
