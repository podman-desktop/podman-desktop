...
import SelectableExtensionDevelopmentFolderActionStop from './SelectableExtensionDevelopmentFolderActionStop.svelte';
...
test('Expect stop action being displayed if there is an extension started', async () => {
  render(SelectableExtensionDevelopmentFolderActionStop, { extensionFolder: extensionFolderWithExtensionStarted });
...
test('Expect stop action being hidden if extension is stopped', async () => {
  render(SelectableExtensionDevelopmentFolderActionStop, { extensionFolder: extensionFolderWithExtensionStopped });
  const stopButton = screen.getByRole('button', { name: 'Stop the extension' });
  expect(stopButton).toBeInTheDocument();
