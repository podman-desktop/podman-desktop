...
import DevelopmentExtensionTableActionsColumn from './DevelopmentExtensionTableActionsColumn.svelte';
...
test('Expect start action being displayed', async () => {
  render(DevelopmentExtensionTableActionsColumn, { object: extensionFolderWithExtensionStopped });
  const startButton = screen.getByRole('button', { name: 'Start the extension' });
  expect(startButton).toBeInTheDocument();
});

test('Expect stop action being displayed', async () => {
  render(DevelopmentExtensionTableActionsColumn, { object: extensionFolderWithExtensionStarted });
