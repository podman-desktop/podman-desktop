...
import SelectableExtensionDevelopmentFolderActionStart from './SelectableExtensionDevelopmentFolderActionStart.svelte';
...
test('Expect start action being displayed if there is an extension stopped', async () => {
  render(SelectableExtensionDevelopmentFolderActionStart, { extensionFolder: extensionFolderWithExtensionStopped });
...
test('Expect start action being hidden if extension is started', async () => {
  render(SelectableExtensionDevelopmentFolderActionStart, { extensionFolder: extensionFolderWithExtensionStarted });
  const stopButton = screen.getByRole('button', { name: 'Start the extension' });
  expect(stopButton).toBeInTheDocument();
