...
import {
  getNavigationDragName,
  LONG_PRESS_MS,
  navigationDragState,
} from './stores/navigation/navigation-drag-state.svelte';
import type { NavigationRegistryEntry } from './stores/navigation/navigation-registry';
import {
  findNavigationEntryByLink,
  flattenNavigationEntries,
  getNavigationEntryIdentifier,
  navigationRegistry,
  promoteToNavbar,
...
let dragGhost = $derived.by((): { name: string; icon?: DragGhostIcon } | undefined => {
  ...
  if (payload) {
    const found = findNavigationEntryByLink($navigationRegistry, payload.link);
    return {
      name: getNavigationDragName(payload),
...
function commitExternalDrop(): void {
  ...
  const visualIds = allVisibleEntries.map(getNavigationEntryIdentifier);
  const existingEntry = allVisibleEntries.find(entry => entry.link === payload.link);
  const orderId = existingEntry ? getNavigationEntryIdentifier(existingEntry) : getNavigationDragName(payload);
  ...
  if (!findNavigationEntryByLink($navigationRegistry, payload.link)) {
    promoteToNavbar(payload);
  }
  const newPos = newOrder.indexOf(orderId) + 1;
  const message = wasInMainNav
    ? `Moved ${getNavigationDragName(payload)} to position ${newPos} of ${newOrder.length}`
    : `Pinned ${getNavigationDragName(payload)} to main navigation at position ${newPos} of ${newOrder.length}`;
...
function onDragUp(): void {
  if (dragIndex === undefined) return;
  if (dropIndex !== undefined && !isNoOpInsert(dragIndex, dropIndex)) {
...
function commitReorder(fromIndex: number, insertIndex: number): void {
  ...
  const movedId = getNavigationEntryIdentifier(movedEntry);
  const newOrder = applyInsertSlotReorder(
    allVisibleEntries.map(getNavigationEntryIdentifier),
    movedId,
    fromIndex,
    insertIndex,
  );
  ...
  navigationDragState.announcement = `Moved ${movedEntry.name} to position ${newPos} of ${newOrder.length}`;
}
        ...
        {#each allVisibleEntries as entry, i (entry.link)}
          <div
            ...
            <NavRegistryEntry
              ...
              titleTooltip={isReorderDragging ? undefined : `Hold to reorder. ${modifierC}Arrow to move`} />
          </div>
        {/each}
        {#if showDropLine}
          <div
