...
let flatNavigationEntries = $derived(navigationUtils.flattenNavigationEntries($navigationRegistry));
...
let dragGhost = $derived.by((): { name: string; icon?: DragGhostIcon } | undefined => {
  ...
  if (payload) {
    ...
    return {
      name: navigationUtils.getNavigationDragName(payload),
...
function commitExternalDrop(): void {
  ...
  const visualIds = allVisibleEntries.map(entry => entry.name);
  const existingEntry = allVisibleEntries.find(entry => entry.link === payload.link);
  const orderId = existingEntry?.name ?? navigationUtils.getNavigationDragName(payload);
  const wasInMainNav = existingEntry !== undefined;
  const newOrder = applyExternalInsert(visualIds, orderId, dropIndex);
  setNavigationItemOrder(newOrder, payload);
  const newPos = newOrder.indexOf(orderId) + 1;
  const message = wasInMainNav
    ? `Moved ${navigationUtils.getNavigationDragName(payload)} to position ${newPos} of ${newOrder.length}`
    : `Pinned ${navigationUtils.getNavigationDragName(payload)} to main navigation at position ${newPos} of ${newOrder.length}`;
...
function commitReorder(fromIndex: number, insertIndex: number): void {
  ...
  const movedId = movedEntry.name;
  const newOrder = applyInsertSlotReorder(
    allVisibleEntries.map(entry => entry.name),
    movedId,
    fromIndex,
    insertIndex,
  );
