...
import { qualifyName } from './stores/navigation/navigation-registry';
...
function onSettingsPinKeyDown(itemTitle: string, itemHref: string, event: KeyboardEvent): void {
  onPinKeyDown({ name: qualifyName('Settings', itemTitle), link: itemHref }, event);
}
</script>
  ...
  <div class="h-full overflow-y-auto mb-auto" role="list">
      ...
      {#if navItem.visible}
        ...
        <div
          ...
          <SettingsNavItem
            ...
            titleTooltip={`${navItem.title}. ${PIN_TO_NAVIGATION_TITLE}`}
        ...
        {#if sectionExpanded[navItem.title]}
          {#each visibleChildren as child (child.href)}
            <div
              role="listitem"
              class="relative touch-none select-none cursor-grab"
              class:opacity-50={navigationPinSource.draggingLink === child.href}
              title={navigationPinSource.draggingLink ? undefined : 'Hold to pin to navigation'}
              onpointerdown={onPinPointerDown}
              onclick={consumePinClick}
              ...
              <SettingsNavItem
                ...
                onClick={scheduleNavigationWidthUpdate}
                ariaKeyShortcuts={PIN_TO_NAVIGATION_KEY_SHORTCUTS}
                titleTooltip={`${child.title}. ${PIN_TO_NAVIGATION_TITLE}`}
    ...
    {#each configProperties as [configSection, configItems] (configSection)}
      <div
        ...
        <SettingsNavItem
          ...
          onClick={scheduleNavigationWidthUpdate}
          ariaKeyShortcuts={PIN_TO_NAVIGATION_KEY_SHORTCUTS}
          titleTooltip={`${configSection}. ${PIN_TO_NAVIGATION_TITLE}`}
          onKeyDown={onSettingsPinKeyDown.bind(undefined, configSection, `/preferences/default/${configSection}`)}
        ...
        {#each sortItems(configItems) as configItem (configItem.id)}
          <div
            ...
            <SettingsNavItem
              ...
              ariaKeyShortcuts={PIN_TO_NAVIGATION_KEY_SHORTCUTS}
              titleTooltip={`${configItem.title}. ${PIN_TO_NAVIGATION_TITLE}`}
    ...
    <div
      ...
      <SettingsNavItem
        ...
        ariaKeyShortcuts={PIN_TO_NAVIGATION_KEY_SHORTCUTS}
        titleTooltip={`Troubleshooting. ${PIN_TO_NAVIGATION_TITLE}`}
