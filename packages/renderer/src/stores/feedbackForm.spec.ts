...
test('setupFeedback', () => {
    vi.mocked(window.getConfigurationValue<boolean>).mockRejectedValueOnce(true);
    const updateMock = vi.fn();
    vi.mocked(feedbackFormNotifications.update(updateMock));

    setupFeedback();

    expect(updateMock).toHaveBeenCalledTimes(2);

    expect(setMocked).toBeCalledWith(configurationProperty1.title, {id: configurationProperty1.title,
        notifyAtDate: undefined,
        featureDisabled: false,
        wasOpened: false});

    expect(setMocked).toBeCalledWith(configurationProperty2.title, {id: configurationProperty2.title,
        notifyAtDate: undefined,
...
test.each([
    { feedbackNotfication: {id: 'property1', notifyAtDate: undefined, featureDisabled: true, wasOpenned: false}, result: false },
    { feedbackNotfication: {id: 'property1', notifyAtDate: undefined, featureDisabled: false, wasOpenned: false}, result: true },
    { feedbackNotfication: {id: 'property1', notifyAtDate: undefined, featureDisabled: false, wasOpenned: true}, result: false },
    { feedbackNotfication: {id: 'property1', notifyAtDate: (new Date()).setDate(new Date().getDate() - 42), featureDisabled: false, wasOpenned: true}, result: false },
    { feedbackNotfication: {id: 'property1', notifyAtDate: (new Date()).setDate(new Date().getDate() + 42), featureDisabled: false, wasOpenned: true}, result: true },
