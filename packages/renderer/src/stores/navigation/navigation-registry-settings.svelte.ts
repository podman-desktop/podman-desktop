...
export function createNavigationSettingsEntries(
  properties: IConfigurationPropertyRecordedSchema[] = [],
): NavigationRegistryEntry[] {
  const parentIds = new Set<string>();
  const dynamicEntries: NavigationRegistryEntry[] = [];
  for (const property of properties) {
    if (
      property.scope !== CONFIGURATION_DEFAULT_SCOPE ||
      property.hidden ||
      property.parentId === 'preferences' ||
      !property.parentId.startsWith('preferences.')
    ) {
      continue;
    }

    if (parentIds.has(property.parentId)) {
      continue;
    }
    parentIds.add(property.parentId);
    dynamicEntries.push(toEntry(property.title, `/preferences/default/${property.parentId}`));
  }

  return [...settingsEntries, ...dynamicEntries];
}
