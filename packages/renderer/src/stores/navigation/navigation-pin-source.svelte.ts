...
export const PIN_TO_NAVIGATION_KEY_SHORTCUTS = 'Control+ArrowLeft Meta+ArrowLeft';
export const PIN_TO_NAVIGATION_TITLE = 'Hold to pin to navigation. Ctrl/Cmd+Left Arrow to pin';
...
export function onPinKeyDown(payload: DragPayload, event: KeyboardEvent): void {
  if (event.key !== 'ArrowLeft' || (!event.ctrlKey && !event.metaKey)) {
    return;
  }
  event.preventDefault();
  const result = pinToNavbar(payload.name, payload.link);
  navigationDragState.announcement = result.changed
    ? `Pinned ${payload.name} to main navigation at position ${result.position} of ${result.total}`
    : `${payload.name} is already in main navigation at position ${result.position} of ${result.total}`;
}
