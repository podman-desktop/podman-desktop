...
import { beginPin, consumePinClick, onPinKeyDown, onPinPointerDown, resetPin } from './navigation-pin-source.svelte';
...
describe('consumePinClick', () => {
  ...
  test('does not swallow clicks after a still beginPin with no movement', () => {
    ...
    beginPin({ parentName: 'Kubernetes', name: 'Nodes', link: '/kubernetes/nodes' });
...
describe('onPinPointerDown', () => {
  test('ignores non-primary buttons', () => {
    ...
    beginPin({ parentName: 'Settings', name: 'X', link: '/x' });
...
describe('onPinKeyDown', () => {
  test('pins with Ctrl+ArrowLeft and announces the result', () => {
    ...
    onPinKeyDown({ parentName: 'Settings', name: 'Resources', link: '/preferences/resources' }, event);
