...
test('resolveItemOrderIndex uses links only', () => {
  expect(resolveItemOrderIndex(['/pods'], '/pods')).toBe(0);
  expect(resolveItemOrderIndex(['Pods'], '/kubernetes/pods')).toBeUndefined();
  expect(resolveItemOrderIndex(['/kubernetes/pods'], '/kubernetes/pods')).toBe(0);
  expect(resolveItemOrderIndex(['/kubernetes/pods'], '/pods')).toBeUndefined();
});
...
test('collecItem includes unpinned settings/submenu children without index for context-menu pin', () => {
  const settingsChild = makeLeafEntry('Settings > Resources', '/preferences/resources', { parentName: 'Settings' });
