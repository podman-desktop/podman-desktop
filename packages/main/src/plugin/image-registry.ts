...
export class ImageRegistry {
  ...
  protected async getManifestFromURL(
    ...
    try {
      ...
      digest = response.headers['docker-content-digest'] as string | undefined;
    } catch (requestErr) {
      if (
        requestErr instanceof RequestError &&
        (requestErr.response?.statusCode === 401 || requestErr.response?.statusCode === 403)
  ...
  async checkImageUpdateStatus(
    ...
    try {
      ...
      if (hasLocalTag) {
        // Local images can't be updated from registry - this is expected behavior, not an error
        return {
          status: 'normal',
          updateAvailable: false,
          message: 'Image is likely local and cannot be updated from a remote registry',
        };
      } else {
