...
beforeEach(() => {
  vi.clearAllMocks();
  vi.useFakeTimers();
  vi.setSystemTime(new Date(Date.UTC(2_012, 11, 21, 0, 0, 0)));
...
describe('createTempFile', () => {
  ...
  test('tracks multiple temporary files', async () => {
    ...
    vi.setSystemTime(new Date(Date.UTC(2_012, 11, 21, 0, 0, 1)));
...
describe('cleanup', () => {
  test('removes all tracked temporary files', async () => {
    ...
    vi.setSystemTime(new Date(Date.UTC(2_012, 11, 21, 0, 0, 1)));
    await tempFileService.createTempFile('content2');
    vi.setSystemTime(new Date(Date.UTC(2_012, 11, 21, 0, 0, 2)));
