...
import { CONFIGURATION_DEFAULT_SCOPE, CONFIGURATION_MANAGED_DEFAULTS_SCOPE } from '/@api/configuration/constants.js';
...
test('should return admin defaults scope for admin defaults configuration', () => {
  ...
  expect(config.getConfigurationKey()).toBe(CONFIGURATION_MANAGED_DEFAULTS_SCOPE);
});
...
test('should get values from admin defaults scope configuration', () => {
  ...
  const map = new Map<string, { [key: string]: unknown }>();
  map.set(CONFIGURATION_MANAGED_DEFAULTS_SCOPE, { 'test.section.key': 'adminValue' });
